...
class AgentEnrolmentPredicate @Inject()(enrolmentURI: URI, authorisationService: AuthorisationService,
  ...
  def apply(authContext: AuthContext, request: Request[AnyContent]): Future[PageVisibilityResult] = {
    ...
    for {
      ...
      enrolled <- enrolmentToCGTCheck.checkAgentEnrolments(enrolments)
