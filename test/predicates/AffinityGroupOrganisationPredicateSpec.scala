...
class AffinityGroupOrganisationPredicateSpec extends UnitSpec with WithFakeApplication with MockitoSugar {
  ...
  def mockedService(response: Option[AuthorisationDataModel], nino: Option[Nino]): AuthorisationService = {
    ...
    when(mockConnector.getAuthResponse()(ArgumentMatchers.any())).thenReturn(Future.successful(response))
  ...
  "InstantiatingAffinityGroupOrganisationPredicate" when {
    ...
    "supplied with an authContext with a organisation credential should return true for page visibility" in {
